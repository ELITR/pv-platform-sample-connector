...
static void printUsage (char *prgName) {
  printf ("\n");
  printf ("\nNAME\n\t%s - Segmenting Audio Recorder\n", prgName);
  printf ("\nSYNOPSIS\n\t%s [OPTION]\n", prgName);
  printf ("\nDESCRIPTION\n"
      "\tThis is a example implementation of a segmenting audio recorder backend which connects to\n"
...
int initCallback (MCloud *cP, MCloudPacket *p, void *userData) {
	...
	segmentDirName[0] = '\0';
	sprintf(segmentDirName, "%s/segments-%s", outputDir, date);
	make_directory(segmentDirName);
...
int main (int argc, char * argv[]) {
  ...
  */
  ...
  mcloudAddService (cloudP, "Segmenting Audio Recorder", "audio recorder", inputFingerPrint, inputType, outputFingerPrint, outputType, NULL);
